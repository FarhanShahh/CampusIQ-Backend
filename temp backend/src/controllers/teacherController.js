...
const getTeacherAnalytics = async (req, res) => {
  try {
    ...
    const avgMarks =
      marksCount > 0 ? (totalMarksSum / marksCount).toFixed(2) : 0;
    const avgInternalTotal =
      marksCount > 0
        ? (
            studentData.reduce((acc, s) => acc + s.internalTotal, 0) /
            marksCount
          ).toFixed(2)
        : 0;
    const avgAttendanceRate =
      students.length > 0 ? Math.round(totalAttRateSum / students.length) : 0;
...
const getMarksTemplate = async (req, res) => {
  try {
    ...
    if (subject.subjectType === "Theory+Lab+Practical")
      headers.push("Practical Oral");
    ...
    const filePath = path.join(
      __dirname,
      "../uploads",
      `Template_${subject.subjectCode}.xlsx`,
    );
...
const uploadMarks = async (req, res) => {
  try {
    ...
    for (const row of data) {
      ...
      if (subject.subjectType !== "Lab+Practical") {
        for (let i = 1; i <= 6; i++)
          marks.internalTests.push(Number(row[`Test ${i}`]) || 0);
      }
      if (subject.subjectType !== "Theory") {
        for (let i = 1; i <= 10; i++)
          marks.experiments.push(Number(row[`Exp ${i}`]) || 0);
        for (let i = 1; i <= 2; i++)
          marks.assignments.push(Number(row[`Assignment ${i}`]) || 0);
      }
      if (subject.subjectType === "Theory+Lab+Practical")
        marks.practicalOral = Number(row["Practical Oral"]) || 0;
      if (subject.subjectType !== "Lab+Practical")
        marks.theory = Number(row["Theory Exam"]) || 0;

      // Calculate totals
      const internalTotal =
        (marks.internalTests.reduce((a, b) => a + b, 0) / 60) * 20;
